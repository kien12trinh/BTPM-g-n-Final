...
class SavingsService:
    """
    Business logic layer for Saving Goals.
    Repositories handle DB access; this layer enforces rules.
    """
    ...
    # ── Goals ─────────────────────────────────────────────────────
    def list_goals(self):
        return self._goals.get_all_ordered()
    ...
    def create_goal(self, name: str, target_amount: int, icon: str, deadline=None):
    ...
    def update_goal(self, goal_id: int, **kwargs):
        ...
        if "target_amount" in kwargs:
            if kwargs["target_amount"] <= 0:
                raise ValueError("Số tiền mục tiêu phải lớn hơn 0")
            # Don't allow shrinking target below what's already saved
            if kwargs["target_amount"] < goal.saved_amount:
                raise ValueError(
                    "Số tiền mục tiêu không thể nhỏ hơn số tiền đã tiết kiệm"
                )
    ...
    # ── Deposits ──────────────────────────────────────────────────
    def list_deposits(self, goal_id: int):
        self.get_goal(goal_id)  # raises if not found
        return self._deposits.get_by_goal(goal_id)

    def add_deposit(self, goal_id: int, amount: int, deposit_date=None, note=None):
        goal = self.get_goal(goal_id)
    ...
    def delete_deposit(self, goal_id: int, deposit_id: int):
